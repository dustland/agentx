...
AgentX provides built-in multi-tenant support through workspace isolation and user-scoped API access. This design ensures that different users can securely share the same AgentX instance while maintaining complete data isolation.

## Overview

Multi-tenancy in AgentX is implemented as a lightweight extension to the workspace system, providing user isolation without requiring complex authentication infrastructure. This approach maintains backward compatibility while enabling secure multi-user deployments.

### Key Security Principles

- **User Isolation**: Each user's tasks and data are completely isolated
- **Workspace Segregation**: File system isolation prevents cross-user access
...
- **Artifact Access**: File system access limited to user's workspace
...
class WorkspaceStorage:
    def __init__(self, base_path, task_id, user_id=None):
        if user_id:
            # Multi-tenant: workspace/{user_id}/{task_id}
            self.workspace_path = Path(base_path) / user_id / task_id
        else:
            # Single-tenant: workspace/{task_id}
...
- **Artifacts**: Git versioning isolated per user workspace
- **Logs**: Execution logs stored in user-specific directories

### Path Traversal Protection

The workspace system includes built-in protection against path traversal attacks:

```python
# Safe: resolves to workspace/alice/task123/file.txt
workspace.get_artifact("file.txt")

# Blocked: attempts to access ../bob/task456/ are prevented
# by Path.resolve() and workspace boundary checks
```
...
# All tasks use legacy workspace structure
...
# Tasks created with user_id use isolated workspaces
...
3. **Migrate existing workspaces** if needed:

```bash
# Migrate legacy workspaces to user-scoped structure
for task_dir in workspace/*/; do
    ...
    mkdir -p "workspace/default/$task_id"
    mv "$task_dir"/* "workspace/default/$task_id/"
    rmdir "$task_dir"
done
```
...
1. **Clean up old workspaces** periodically
...
1. **Track workspace sizes** per user
