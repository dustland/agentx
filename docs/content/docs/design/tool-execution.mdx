...
- **Isolation**:
  ...
  - **Filesystem**: The container is only granted access to the current task's taskspace directory (`/app/taskspace`). It cannot read from or write to any other part of the host filesystem.
