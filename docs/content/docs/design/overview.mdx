...
AgentX enables organisations to decompose complex goals into collaborative **Teams** of agents, each focusing on a well-defined role. A stateful **Lead** governs the workflow, assigns tasks to agents, and executes tools on the agents' behalf, while a **Task Executor** drives the lifecycle of the task itself. The result is a flexible framework that elevates individual agent capabilities into a cohesive, self-optimising system that can learn, adapt, and scale with minimal human intervention.
...
- **Lead-Driven Orchestration**: A stateful Lead governs coordination, planning, and task assignment.
...
graph TD
    ...
    BRAIN["Agent Brain"]
    TM["Tool Manager"]
    WORKSPACE["Workspace<br/>(plan.md, artifacts)"]
...
- **Core Framework**: The central layer containing `TaskExecutor`, `Lead`, `Agent`, and `Brain` components. Core imports FROM subsystems rather than the other way around, implementing true dependency inversion.
...
graph TD
    ...
    subgraph "Core Framework"
        TE["core/<br/>• task.py (TaskExecutor)<br/>• lead.py<br/>• agent.py<br/>• brain.py"]
    end
    ...
    subgraph "Self-Contained Subsystems"
        ...
        STORAGE["storage/<br/>• models.py<br/>• backends.py<br/>• git_storage.py<br/>• workspace.py"]
    ...
    subgraph "External Systems"
        ...
        GIT["Git<br/>• Workspace versioning<br/>• Artifact storage"]
...
- **Stateful:** Maintains task progress in `plan.md` and workspace files
- **Orchestrates:** Decides which agent handles each step
- **Persistent:** Survives across multiple turns and can resume interrupted tasks
- **Two implementations:**
  - `BaseLead`: Abstract class for custom orchestration logic
  - `Lead`: Default implementation with LLM-based intelligent routing
...
- **Reusable:** Same agent can be used by different Leads for different tasks
- **Standard Library:** Framework provides common agents (planner, researcher, writer, reviewer)

**TaskExecutor (The Runtime)**

- **Lifecycle Manager:** Handles workspace setup, tool management, streaming
- **Lead Invoker:** Calls the Lead and provides it with agents and tools
- **Stateless:** Creates new instances per task execution
...
This "fire-and-forget" mode is ideal for production. A client submits a task and waits for a final result, while the `Task Executor` runs the entire multi-agent collaboration, driven by the `Lead`.
...
sequenceDiagram
    ...
    loop Autonomous Loop (driven by TaskExecutor)
        TX->>L: invoke Lead for next step
...
### 4.3 Lead vs Specialist Agents: A Key Distinction
...
- Invoked by the Lead to perform specific micro-tasks
- Return results and forget - no state management

**Example Flow:**

1. Lead receives: "Write a report on AI trends"
2. Lead delegates to Planner Agent: "Create a step-by-step plan"
3. Planner creates `plan.md` with checklist
4. Lead reads plan, assigns "Research AI trends" to Researcher Agent
5. Lead assigns "Write introduction" to Writer Agent
6. Lead continues until all steps complete
...
lead: default # Uses the built-in Lead class
```
