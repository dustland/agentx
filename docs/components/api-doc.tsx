...
export function ApiDoc() {
  ...
  useEffect(() => {
    ...
    fetch('http://localhost:8000/openapi.json')
      ...
      .catch(() => {
        setError('Unable to connect to API server. Please ensure AgentX is running on port 8000.')
        setIsLoading(false)
      })
  }, [])
    ...
    return (
      <div className="flex items-center justify-center h-96">
        <div className="text-center">
          ...
          <p className="text-gray-600 dark:text-gray-400">Connecting to API server...</p>
        </div>
      </div>
    )
  }
    ...
    return (
      <div className="rounded-lg border border-red-200 bg-red-50 dark:border-red-800 dark:bg-red-950 p-6">
        ...
        <div className="bg-gray-900 dark:bg-gray-800 rounded p-3 font-mono text-sm text-gray-100">
          <span className="text-green-400">$</span> uv run start --port 8000
        </div>
      </div>
    )
  }
  ...
  return (
    <div className="flex flex-col h-full">
      ...
      <iframe 
        ...
        src={viewMode === 'redoc' 
          ? isDarkMode 
            ? 'http://localhost:8000/redoc?theme=https://raw.githubusercontent.com/dilanx/redark/main/redark.json'
            : 'http://localhost:8000/redoc'
          : 'http://localhost:8000/docs'
        }
        className="w-full flex-1 border-0"
        style={{
          filter: isDarkMode && viewMode === 'swagger' ? 'invert(0.93) hue-rotate(180deg) contrast(0.9)' : 'none',
