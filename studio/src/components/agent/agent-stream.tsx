...
export function AgentStream({ taskId, apiUrl = process.env.NEXT_PUBLIC_AGENTX_API_URL || 'http://localhost:8000', onToolCall }: AgentStreamProps) {
          ...
          {Object.entries(agentStatuses).map(([agentId, status]) => (
            <Badge key={agentId} variant="outline" className="text-xs">
              <div className={`w-2 h-2 rounded-full mr-1 ${
                ...
                status.status === 'completed' ? 'bg-green-500' : 'bg-gray-500'
              }`} />
                ...
                <div key={message.id} className="flex gap-3">
                  ...
                  <div className="flex-1 space-y-1">
                    <div className="flex items-center gap-2">
                      <span className="font-medium text-sm">{message.agent_id}</span>
                    ...
                    {message.metadata?.tool_calls && message.metadata.tool_calls.map((tool: any, idx: number) => (
                      <Card key={idx} className="mt-2 p-2 bg-secondary/20 cursor-pointer hover:bg-secondary/30"
                        onClick={() => {
                          const toolCall: ToolCall = {
